...
#  character string:
#
student_number = 2020338038
student_name   = 'Rj Avro'
#
...
main_window.title('Robotics Fact Checking') #put a title here
...
def fnc_show_source():
    ...
    msg = Message(status_frame, width=360, text='Source '+m+' selected!',
...
img_frame = Frame(master=main_window, borderwidth=4, border=2, background='red')

status_frame = LabelFrame(master=text_frame, text='System Status', background=bg_color,
                          font=font_face, fg='blue',labelanchor='nw', padx=5, pady=5,
...
data_frame = LabelFrame(master=text_frame, text='Data Source', background=bg_color,
                        font=font_face,labelanchor='nw', padx=5, pady=5,
                        borderwidth=3, border=4, width=250, height=160)
...
status_frame.grid(row=0, column=0, sticky='w', pady=5)
data_frame.grid(row=1, column=0, sticky='w', pady=5)
rating_frame.grid(row=2, column=0, sticky='w', pady=5)
...
r = StringVar(value='No Option Selected')
Radiobutton(data_frame, background=bg_color, text=' Source - A', font=(font_face, 13),
            variable=r, value='A', command=fnc_show_source).grid(row=0, column=0)

Radiobutton(data_frame, background=bg_color, text=' Source - B', font=(font_face, 13),
            variable=r, value='B', command=fnc_show_source).grid(row=1, column=0)

Radiobutton(data_frame, background=bg_color, text=' Source - C', font=(font_face, 13),
            variable=r, value='C', command=fnc_show_source).grid(row=2, column=0)
...
image = PhotoImage(file='./Robot.png') #image location here
